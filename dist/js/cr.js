...
function tdLeftClicked(elTd) {
    //CR: not a very good function name
...
function showHint() {
    //CR: nicely done, though not what we asked for. The task was getting 9 cells open for a second when clicking a hint and then clicking a cell.
...
function plantBombsOnGBoard(bombsAmount) {
    //CR: could've used an array with empty spaces instead of going random
    var plantCount = 0
...
function getStrHtmlTableWithIds(rows, cols) {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            currRowStrHtml += `<td id="${i}_${j}" onclick="pressStartGame(this),tdLeftClicked(this)" oncontextmenu="tdRightClicked(this)" onmousedown="applyCSSOnSmiley(event),cellCSS(event)" onmouseup="applyCSSOnSmiley(event)" ></td>`
        }
        currRowStrHtml += `</tr>`
